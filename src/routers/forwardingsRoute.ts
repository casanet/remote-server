    ...
    app.get('/API/remote/status', async (req: Request, res: Response) => {
      try {
        ...
        try {
          ...
          forwardUserSession = (await expressAuthentication(req, [SystemAuthScopes.forwardScope])) as ForwardSession;
        } catch (error) {
          res.status(401).send({ responseCode: 4001 } as ErrorResponse);
          return;
        }

        const remoteServerStatus: RemoteConnectionStatus = (await ChannelsSingleton.connectionStatus(
          forwardUserSession.server,
        ))
          ? 'connectionOK'
